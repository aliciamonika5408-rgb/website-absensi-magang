...
import { submitAbsenPulang, getTodayAttendance } from "../services/db";
import { getCurrentGPSLocation, validateLocationRadius, TARGET_LOCATION } from "../services/locationService";
import confetti from "canvas-confetti";

export default function AbsenPulang({ user, onNavigate }) {
  ...
  useEffect(() => {
    ...
    if (record && record.jamPulang && record.jamPulang !== "-") {
  ...
  const handleKirimAbsensi = () => {
    ...
    const record = submitAbsenPulang(user, keterangan, location);
    setTodayRecord(record);
    setIsSubmitted(true);
  ...
  return (
    <div className="animate-fade-in" style={{ maxWidth: "700px", margin: "0 auto", paddingBottom: "2rem" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "1.5rem" }}>
        ...
        <div>
          <h1 style={{ fontSize: "1.5rem", fontWeight: "800", color: "#0f172a" }}>Halaman Absen Pulang</h1>
          <p style={{ fontSize: "0.875rem", color: "#64748b" }}>Isi catatan kegiatan/keterangan & verifikasi lokasi GPS Anda untuk melakukan presensi pulang.</p>
        </div>
      </div>
      ...
      >
        {isSubmitted || (todayRecord && todayRecord.jamPulang && todayRecord.jamPulang !== "-") ? (
          <div style={{ textAlign: "center", padding: "1.5rem 0.5rem" }}>
            ...
            <h2 style={{ fontSize: "1.5rem", fontWeight: "800", color: "#0f172a" }}>Absensi Pulang Berhasil!</h2>
            <p style={{ fontSize: "0.95rem", color: "#64748b", marginTop: "0.5rem" }}>
              Terima kasih telah magang dengan baik hari ini, <strong>{user.nama}</strong>. Hati-hati di jalan!
            </p>
            ...
            >
              ...
              <div>
                <span style={{ fontSize: "0.75rem", color: "#64748b", fontWeight: "600", display: "block" }}>JAM PULANG</span>
                <p style={{ fontSize: "0.95rem", fontWeight: "800", color: "#dc2626", marginTop: "2px" }}>
                  {activeRecord?.jamPulang || timeStr}
                </p>
              </div>
              <div style={{ gridColumn: "1 / -1" }}>
                <span style={{ fontSize: "0.75rem", color: "#64748b", fontWeight: "600", display: "block" }}>CATATAN PULANG</span>
                <p style={{ fontSize: "0.9rem", fontWeight: "600", color: "#334155", backgroundColor: "#ffffff", padding: "0.6rem 0.8rem", borderRadius: "0.5rem", border: "1px solid #e2e8f0", marginTop: "4px" }}>
                  {activeRecord?.keteranganPulang || keterangan || "-"}
                </p>
              </div>
              <div style={{ gridColumn: "1 / -1" }}>
          ...
          <>
            ...
            >
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                ...
                <div>
                  <span style={{ fontSize: "0.75rem", color: "#64748b", fontWeight: "600", display: "block" }}>🕒 Jam Pulang (otomatis)</span>
                  <span style={{ fontSize: "1rem", fontWeight: "800", color: "#dc2626", fontFamily: "monospace" }}>{timeStr}</span>
                </div>
              </div>
            </div>
            ...
            <div style={{ marginBottom: "1.5rem" }}>
              <label
                htmlFor="keterangan-absensi-pulang"
                style={{
                  display: "flex",
                  alignItems: "center",
                  gap: "0.4rem",
              ...
              >
                ...
                Catatan / Keterangan Absensi Pulang <span style={{ color: "#dc2626" }}>*</span>
              </label>
              <textarea
                id="keterangan-absensi-pulang"
                rows={3}
                value={keterangan}
                onChange={(e) => setKeterangan(e.target.value)}
                placeholder="Masukkan keterangan kegiatan atau absensi pulang..."
                style={{
                  width: "100%",
            ...
            <button
              ...
              style={{
                width: "100%",
                backgroundColor: isFormValid ? "#dc2626" : "#cbd5e1",
                ...
                justifyContent: "center",
                gap: "0.6rem",
                boxShadow: isFormValid ? "0 8px 20px rgba(220, 38, 38, 0.3)" : "none",
                opacity: isFormValid ? 1 : 0.7,
                transition: "all 0.2s ease"
              }}
            >
              <Check size={20} /> ✅ Kirim Absensi Pulang
            </button>
          </>
        )}
      </div>
